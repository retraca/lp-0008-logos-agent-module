...
# Every command shown is the REAL command, run live.
set +u +e
...
clear; echo
echo -e "${BOLD}   USE CASE 3 — a paid skill marketplace, no human in the loop${N}"; sleep 3

hd "14 · A peer advertises a skill, with a price" "F7/F8 — agents discover each other via A2A cards"
why "A second agent publishes its A2A card: it can run a job, for 5 LEZ."
...
why "5 LEZ is under the 50 limit, so the agent should just pay it, no human."
...
why "Live daemon: the wallet fires and the transfer proof runs." 3
sleep 4
...
why "And if the price were 80, over the 50 limit, the agent would not pay."
...
echo; echo; echo; echo -e "${B}▌ Every success criterion — met${N}"; echo; sleep 2.5
...
o "${G}✓${N}${CY} F11·U1·U2·S2·S3·S4  docs · app · CI · README        ${DIM}step 4,13${N}"
echo; sleep 4
