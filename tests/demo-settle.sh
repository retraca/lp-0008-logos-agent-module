#!/usr/bin/env bash
# tests/demo-settle.sh — LP-0008 demo PART B: the agent hires + pays a peer, by itself.
# Code-forward; headers numbered to match docs/VIDEO_NARRATION.md (sections 8-10).
set +u +e
...
clear; echo
echo -e "${BOLD}   USE CASE — a paid skill marketplace, no human in the loop${N}"; sleep 2.2

hd "8 · A peer advertises a skill, with a price"
...
o "    skill   compute.run"
o "    price   5 LEZ"
o "    id      ${BID:0:34}…"; sleep 2.4

hd "9 · The agent hires it — and pays, by itself"
AB=$("$LC" call lez_wallet_module balance 2>/dev/null | grep -o '[0-9]*' | head -1)
o "  before:  agent ${AB} LEZ    peer B 0 LEZ"; sleep 1.2
...
# poll until the transfer actually settles (proof + on-chain land)
...
o "  settled: −5 from the agent's own funds, +5 to the peer"; sleep 2.6

hd "10 · Over the limit? It asks first"
run "agent_task at price 80  (limit is 50)"
o "  80 > 50  →  pending_approval"
o "  reason: spend exceeds autonomous threshold — NOT sent"; sleep 2.6
