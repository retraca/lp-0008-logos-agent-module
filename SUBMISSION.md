...
| F8 | Two or more agents discover, execute A2A task lifecycle, transfer LEZ payment autonomously without owner intervention | **DONE** | `docs/EVIDENCE_LOCAL.md` §1a–1e — agent_discover → agent_task (price resolved from card) → real-proof payment settled; tx `96724ec5…`; sender/recipient balances independently verified via RPC |
| F9 | At least 3 illustrative use cases demonstrated end-to-end on LEZ testnet | **PARTIAL** | Blockchain skill category: send + receive on testnet (DONE, `docs/TESTNET_EVIDENCE.md`). Storage and Messaging skill round-trips (CID retrieval, group relay) need a libp2p peer network — Storage and Messaging agents are funded and addressable but their skill settlement is not demonstrated on single-node infra. Infra dependency, not a code gap. |
